...
Shell drivers that orchestrate full or partial sweeps over the EmoPhone three-setting benchmark. They invoke the top-level Python runners ([`../execute_benchmark.py`](../execute_benchmark.py), [`../execute_cross_dataset.py`](../execute_cross_dataset.py)) with the per-setting hyper-parameter settings documented in [`../benchmark/`](../benchmark/).
