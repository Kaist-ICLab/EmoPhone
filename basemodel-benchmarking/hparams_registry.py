...
def get_hparams(algorithm, dataset, backbone="MLP"):
    ...
    if algorithm in ["MLP", "ResNet"]:
        ...
        hparams["weight_decay"] = lambda trial: trial.suggest_float(
        ...
        if algorithm == "IRM":
            hparams["irm_lambda"] = lambda trial: trial.suggest_float(
                "irm_lambda", 1e-1, 1e5, log=True
            )
            hparams["irm_penalty_anneal_iters"] = lambda trial: _pow10_uniform_int(
                trial, "irm_penalty_anneal_iters", 0, 4
            )

        if algorithm == "VREx":
            hparams["vrex_lambda"] = lambda trial: trial.suggest_float(
                "vrex_lambda", 1e-1, 1e5, log=True
            )
            hparams["vrex_penalty_anneal_iters"] = lambda trial: _pow10_uniform_int(
                trial, "vrex_penalty_anneal_iters", 0, 4
            )

        if algorithm == "GroupDRO":
            hparams["groupdro_eta"] = lambda trial: trial.suggest_float(
                "groupdro_eta", 1e-3, 1e-1, log=True
            )

        if algorithm == "MixStyle":
            hparams["mixstyle_alpha"] = lambda trial: trial.suggest_float(
                "mixstyle_alpha", 0.1, 0.5
            )
            hparams["mixstyle_p"] = lambda trial: trial.suggest_float("mixstyle_p", 0.1, 0.9)
            hparams["mixstyle_mix"] = lambda trial: trial.suggest_categorical(
                "mixstyle_mix", ["random", "crossdomain"]
            )

        if algorithm == "MLDG":
            hparams["mldg_beta"] = lambda trial: trial.suggest_float(
                "mldg_beta", 1e-1, 10.0, log=True
            )
            hparams["n_meta_test"] = lambda trial: trial.suggest_categorical("n_meta_test", [1, 2])

        if algorithm == "MASF":
            hparams["masf_inner_lr"] = lambda trial: trial.suggest_float(
                "masf_inner_lr", 1e-5, 1e-2, log=True
            )
            hparams["masf_metric_lr"] = lambda trial: trial.suggest_float(
                "masf_metric_lr", 1e-5, 1e-2, log=True
            )
            hparams["masf_metric_weight"] = lambda trial: trial.suggest_float(
                "masf_metric_weight", 1e-4, 1e-2, log=True
            )
            hparams["masf_margin"] = lambda trial: trial.suggest_float("masf_margin", 0.2, 2.0)
            hparams["masf_temperature"] = lambda trial: trial.suggest_float(
                "masf_temperature", 1.0, 5.0
            )
            hparams["masf_metric_dim"] = lambda trial: trial.suggest_categorical(
                "masf_metric_dim", [64, 128, 256]
            )

        if algorithm == "Fish":
            hparams["meta_lr"] = lambda trial: trial.suggest_categorical(
                "meta_lr", [0.05, 0.1, 0.5]
            )
        ...
        if algorithm == "SagNet":
            hparams["sag_w_adv"] = lambda trial: trial.suggest_float(
                "sag_w_adv", 1e-2, 10.0, log=True
            )
        ...
        if algorithm in ["DANN", "CDAN", "ADDA"]:
            hparams["discriminator_lr"] = lambda trial: trial.suggest_float(
                "discriminator_lr", 1e-5, 1e-2, log=True
            )
            hparams["disc_hidden"] = lambda trial: _pow2_uniform_int(trial, "disc_hidden", 6, 10)

        if algorithm == "DANN":
            hparams["trade_off"] = lambda trial: trial.suggest_float(
                "trade_off", 1e-2, 1e2, log=True
            )
        ...
        if algorithm == "CDAN":
            ...
            hparams["cdan_entropy"] = lambda trial: trial.suggest_categorical(
                "cdan_entropy", [True, False]
            )
            hparams["cdan_randomized"] = lambda trial: trial.suggest_categorical(
                "cdan_randomized", [False, True]
            )
            hparams["cdan_randomized_dim"] = lambda trial: trial.suggest_categorical(
                "cdan_randomized_dim", [256, 512, 1024]
            )

        if algorithm == "DeepCORAL":
            hparams["mmd_gamma"] = lambda trial: trial.suggest_float(
                "mmd_gamma", 0.1, 10.0, log=True
            )

        if algorithm == "DAN":
            hparams["dan_trade_off"] = lambda trial: trial.suggest_float(
                "dan_trade_off", 0.1, 10.0, log=True
            )
            hparams["dan_kernel_num"] = lambda trial: trial.suggest_categorical(
                "dan_kernel_num", [3, 5, 7]
            )
            hparams["dan_linear"] = lambda trial: trial.suggest_categorical(
                "dan_linear", [False, True]
            )

        if algorithm == "MCC":
            hparams["mcc_temp"] = lambda trial: trial.suggest_float("mcc_temp", 1.0, 5.0)
            hparams["mcc_trade_off"] = lambda trial: trial.suggest_float(
                "mcc_trade_off", 1e-2, 1e1, log=True
            )
        ...
        if algorithm == "JAN":
            ...
            hparams["jan_kernel_num"] = lambda trial: trial.suggest_categorical(
                "jan_kernel_num", [3, 5, 7]
            )
            hparams["jan_linear"] = lambda trial: trial.suggest_categorical(
                "jan_linear", [False, True]
            )
        ...
        if algorithm == "CBST":
            hparams["cbst_pretrain_epochs"] = lambda trial: trial.suggest_int(
                "cbst_pretrain_epochs", 10, 30
            )
            hparams["cbst_max_iter"] = lambda trial: trial.suggest_categorical(
                "cbst_max_iter", [3, 5, 7]
            )
            hparams["cbst_init_port"] = lambda trial: trial.suggest_float(
                "cbst_init_port", 0.1, 0.3
            )
            hparams["cbst_port_step"] = lambda trial: trial.suggest_float(
                "cbst_port_step", 0.05, 0.2
            )
            hparams["cbst_max_port"] = lambda trial: trial.suggest_float("cbst_max_port", 0.6, 0.9)
            hparams["cbst_retrain_epochs"] = lambda trial: trial.suggest_categorical(
                "cbst_retrain_epochs", [3, 5, 10]
            )
        ...
        if algorithm == "CGDM":
            ...
            hparams["weight_decay"] = lambda trial: trial.suggest_float(
                "weight_decay", 5e-5, 5e-4, log=True
            )
        ...
        hparams["attn_dropout"] = lambda trial: trial.suggest_float("attn_dropout", 0.0, 0.2)
        hparams["ff_dropout"] = lambda trial: trial.suggest_float("ff_dropout", 0.0, 0.2)
        hparams["residual_dropout"] = lambda trial: _zero_or_uniform(
            trial, "residual_dropout", 0.0, 0.1
        )
        ...
        hparams["learning_rate"] = lambda trial: trial.suggest_float(
            "learning_rate", 1e-5, 1.0, log=True
        )
        hparams["max_depth"] = lambda trial: trial.suggest_int("max_depth", 3, 10)
        hparams["min_child_weight"] = lambda trial: trial.suggest_float(
            "min_child_weight", 1e-8, 1e5, log=True
        )
        hparams["subsample"] = lambda trial: trial.suggest_float("subsample", 0.5, 1.0)
        hparams["colsample_bylevel"] = lambda trial: trial.suggest_float(
            "colsample_bylevel", 0.5, 1.0
        )
        hparams["colsample_bytree"] = lambda trial: trial.suggest_float(
