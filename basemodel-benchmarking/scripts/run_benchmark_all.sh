...
for dataset in "D-1" "D-2" "D-3"; do
  ...
  for label in "${labels[@]}"; do
    ...
    for model in "${baselines[@]}"; do
        ...
        echo "------------------------------------------------"
        python3 basemodel-benchmarking/benchmark.py --dataset "$dataset" --label "$label" --model "$model" --hpo_trials 30 --hpo_mode fold1
    done
    ...
    for model in "${tabular_dl[@]}"; do
        ...
        python3 basemodel-benchmarking/benchmark.py --dataset "$dataset" --label "$label" --model "$model" --hpo_trials 30 --hpo_mode fold1 $use_eff
    done
    ...
    for model in "${dg_da_models[@]}"; do
        for backbone in "${backbones[@]}"; do
            ...
            python3 basemodel-benchmarking/benchmark.py --dataset "$dataset" --label "$label" --model "$model" --backbone "$backbone" --hpo_trials 30 --hpo_mode fold1 $use_uda
        done
    done
