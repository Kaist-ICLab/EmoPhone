...
def _load_baseline_metrics(
    dataset_tag: str, results_dir: str = "selected_users_dataset/results"
) -> pd.DataFrame:
    ...
    for path in unique_candidates:
        if path.exists():
            try:
                ...
                logger.info(f"Warning: Failed to load baseline metrics from {path}: {exc}")

    logger.info("Warning: No performance_features.csv found for the requested dataset tag.")
    return pd.DataFrame()
...
def evaluate_cross_user_performance(
    ...
    logger.info(f"=== OTDD-BASED CROSS-USER MODEL EVALUATION ===")
    logger.info(f"Training user: {train_user}")

    # Load dataset
    logger.info("Loading normalized dataset...")
    X, y, users, timestamps, feature_names = load_selected_dataset("reduced")
    logger.info(
        f"Dataset: {X.shape[0]} samples, {X.shape[1]} features, {len(np.unique(users))} users"
    )

    user_ids = np.unique(users)
    if train_user not in user_ids:
        logger.info(f"Error: {train_user} not found in dataset")
        return
    ...
    if distance_matrix_path and os.path.exists(distance_matrix_path):
        logger.info(f"Loading OTDD distances from {distance_matrix_path}")
        ...
        if os.path.exists(default_path):
            logger.info(f"Loading OTDD distances from {default_path}")
    ...
    ranking_df = (
        pd.DataFrame({"user": user_ids, "otdd_distance": distances_from_train})
        .sort_values("otdd_distance")
        .reset_index(drop=True)
    )

    for i, row in ranking_df.iterrows():
        if row["user"] != train_user:
            logger.info(f'{i:2d}. {row["user"]} (OTDD: {row["otdd_distance"]:.6f})')
    ...
    logger.info(f"Loading pre-trained model for {train_user} (dataset tag: {model_dataset_tag})...")
    ...
    logger.info(f"Loaded {len(model_bundle.boosters)} fold models from {model_bundle.model_dir}")
    ...
    logger.info(
        f"Training set performance → AUROC={train_auroc:.4f}, PRAUC={train_prauc:.4f}, ACC={train_acc:.4f}"
    )
    if avg_results:
    ...
    for target_user in user_ids:
        ...
        if len(np.unique(y_target)) < 2:
            logger.info(f"{target_user}: Skipped (only one class)")
            continue
        ...
        logger.info(
            f"{target_user}: AUROC={auroc:.4f}, PRAUC={prauc:.4f}, ACC={accuracy:.4f}, OTDD={otdd_dist:.6f}"
        )
    ...
    logger.info(results_df.round(4).to_string(index=False))
    ...
    if len(results_df) > 1:
        ...
        logger.info(f"Correlation between OTDD distance and AUROC: {auroc_corr:.4f}")
        logger.info(f"Correlation between OTDD distance and PRAUC: {prauc_corr:.4f}")
        logger.info(f"Correlation between OTDD distance and Accuracy: {acc_corr:.4f}")
        ...
        if "delta_auroc" in results_df.columns and results_df["delta_auroc"].notna().sum() > 1:
            ...
            logger.info(
                f"Correlation between OTDD distance and ΔAUROC (self - transfer): {delta_auroc_corr:.4f}"
            )
        ...
        if "delta_prauc" in results_df.columns and results_df["delta_prauc"].notna().sum() > 1:
            ...
            logger.info(
                f"Correlation between OTDD distance and ΔPRAUC (self - transfer): {delta_prauc_corr:.4f}"
            )
        ...
        for _, row in top5.iterrows():
            logger.info(
                f'{row["target_user"]}: OTDD={row["otdd_distance"]:.6f} → AUROC={row["auroc"]:.4f}'
            )
    ...
    if len(results_df) > 0:
        ...
        if best_cv_score is not None:
            logger.info(f"Training user {train_user} stored CV AUROC: {best_cv_score:.4f}")
        else:
            logger.info(f"Training user {train_user} training AUROC (computed): {train_auroc:.4f}")
        logger.info(
            f'Cross-user AUROC - Mean: {results_df["auroc"].mean():.4f}, Std: {results_df["auroc"].std():.4f}'
        )
        logger.info(
            f'Cross-user PRAUC - Mean: {results_df["prauc"].mean():.4f}, Std: {results_df["prauc"].std():.4f}'
        )
        logger.info(
            f'Cross-user Accuracy - Mean: {results_df["accuracy"].mean():.4f}, Std: {results_df["accuracy"].std():.4f}'
        )
        ...
        for _, row in best_auroc.iterrows():
            logger.info(
                f'{row["target_user"]}: AUROC={row["auroc"]:.4f}, OTDD={row["otdd_distance"]:.6f}'
            )
...
if __name__ == "__main__":
    ...
    logger.info(f"Cross-user evaluation for training user: {train_user}")
    if distance_matrix_path:
        logger.info(f"Using distance matrix: {distance_matrix_path}")
    logger.info(f"Using trained model dataset tag: {model_dataset_tag}")
