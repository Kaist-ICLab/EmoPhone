...
def evaluate_cross_user_performance(
    ...
    logger.info(f'Dataset: {X.shape[0]} samples, {X.shape[1]} features, {len(np.unique(users))} users')
    ...
    logger.info(f"Training set performance → AUROC={train_auroc:.4f}, PRAUC={train_prauc:.4f}, ACC={train_acc:.4f}")
    if avg_results:
    ...
    for target_user in user_ids:
        ...
        logger.info(f'{target_user}: AUROC={auroc:.4f}, PRAUC={prauc:.4f}, ACC={accuracy:.4f}, OTDD={otdd_dist:.6f}')
    ...
    if len(results_df) > 1:
        ...
        if 'delta_auroc' in results_df.columns and results_df['delta_auroc'].notna().sum() > 1:
            ...
            logger.info(f'Correlation between OTDD distance and ΔAUROC (self - transfer): {delta_auroc_corr:.4f}')
        ...
        if 'delta_prauc' in results_df.columns and results_df['delta_prauc'].notna().sum() > 1:
            ...
            logger.info(f'Correlation between OTDD distance and ΔPRAUC (self - transfer): {delta_prauc_corr:.4f}')

        logger.info('\\nTop 5 most similar users (by OTDD):')
        top5 = results_df.head(5)
        for _, row in top5.iterrows():
            logger.info(f'{row["target_user"]}: OTDD={row["otdd_distance"]:.6f} → AUROC={row["auroc"]:.4f}')
    ...
    if len(results_df) > 0:
        ...
        logger.info(f'Cross-user AUROC - Mean: {results_df["auroc"].mean():.4f}, Std: {results_df["auroc"].std():.4f}')
        logger.info(f'Cross-user PRAUC - Mean: {results_df["prauc"].mean():.4f}, Std: {results_df["prauc"].std():.4f}')
        logger.info(f'Cross-user Accuracy - Mean: {results_df["accuracy"].mean():.4f}, Std: {results_df["accuracy"].std():.4f}')
        ...
        for _, row in best_auroc.iterrows():
            logger.info(f'{row["target_user"]}: AUROC={row["auroc"]:.4f}, OTDD={row["otdd_distance"]:.6f}')
