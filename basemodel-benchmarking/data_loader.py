...
class BenchmarkDataset:
    def __init__(
        self, dataset_name: str, file_path: str, min_samples: int = 100, min_class_samples: int = 10
    ):
    ...
    def load_data(self):
        ...
        if isinstance(data, (tuple, list)) and len(data) >= 5:
            ...
            for i, name in enumerate(self.feature_names):
                ...
                if (
                    "timestamp" in name_lower
                    or "participant" in name_lower
            ...
            if cols_to_drop:
                logger.info(
                    f"Dropping {len(cols_to_drop)} non-feature columns: {[self.feature_names[i] for i in cols_to_drop[:5]]}..."
                )
                keep_mask = np.ones(self.X.shape[1], dtype=bool)
                keep_mask[cols_to_drop] = False
                self.X = self.X[:, keep_mask]
                self.feature_names = [
                    name for i, name in enumerate(self.feature_names) if keep_mask[i]
                ]
    ...
    def filter_one_month(self):
        ...
        logger.info(
            f"Filtered data to 1 month. Samples: {original_count} -> {len(self.X)} (Removed {original_count - len(self.X)})"
        )
    ...
    def filter_users(self):
        ...
        if dropped_users:
            logger.info(
                f"Dropping {len(dropped_users)} users due to insufficient data: {dropped_users}"
            )
    ...
    def get_raw_stress_aligned(
        self, labels_zip_path: Optional[str] = None, labels_dir: Optional[str] = None
    ) -> np.ndarray:
        ...
        if bad_label_ts > 0:
            raise ValueError(f"Failed to parse {bad_label_ts} timestamps in labels.zip ({member}).")
