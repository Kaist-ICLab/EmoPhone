"""Adversarial Label Group DRO-style MLP with limited Optuna tuning.

This variant mirrors the Group DRO pipeline but treats *labels* as the
groups (robustness to label shift) and keeps the Optuna search space
small to keep runtime manageable.
"""
...
@dataclass
class AdvLabelGroupDROConfig:
    input_dim: int
    hidden_dim_choices: Tuple[int, ...] = (128, 256)
    num_layer_choices: Tuple[int, ...] = (2,)
...
@dataclass
class AdvLabelGroupDROSingleRunResult:
    mode: str
    train_auroc: float
    val_auroc: float
    test_auroc: float
...
class AdvLabelGroupDROBackbone(nn.Module):
...
class AdvLabelGroupDROPipeline:
    def __init__(self, config: AdvLabelGroupDROConfig):
        if optuna is None:
            raise ImportError("optuna is required to run the Adv Label Group DRO pipeline")
        self.config = config
    ...
    def _to_tensor_dataset(self, dataset: ArrayDataset, include_groups: bool) -> Optional[TensorDataset]:
        ...
        if include_groups:
            # Use labels as group identifiers (label-group robustness).
            g = torch.from_numpy(dataset.y.astype(np.int64))
            tensors.append(g)
    ...
    def _evaluate_dataset(self, model: AdvLabelGroupDROBackbone, dataset: Optional[TensorDataset]) -> Dict[str, float]:
        if dataset is None:
    ...
    def _group_loss(self, logits: torch.Tensor, labels: torch.Tensor, groups: Optional[torch.Tensor]) -> torch.Tensor:
        # Groups correspond to labels 0/1; use worst-label loss.
        if groups is None:
    ...
    def _train_epochs(
        self,
        model: AdvLabelGroupDROBackbone,
        ...
        for _ in range(epochs):
            ...
            for batch in train_loader:
                ...
                optimizer.zero_grad()
                logits = model(xb)
                loss = self._group_loss(logits, yb, gb)
                loss.backward()
    ...
    def _build_model(self, hidden_dim: int, num_layers: int) -> AdvLabelGroupDROBackbone:
        model = AdvLabelGroupDROBackbone(
    ...
    def run(
        ...
        *,
    ...
    ) -> List[AdvLabelGroupDROSingleRunResult]:
        ...
        results: List[AdvLabelGroupDROSingleRunResult] = []
        ...
        def _run_variant(mode: str, freeze: bool, scratch: bool, seed_offset: int) -> None:
            ...
            results.append(
                AdvLabelGroupDROSingleRunResult(
                    mode=mode,
                    ...
                    scratch=scratch,
                    hyperparams={
                        "adv_label_dro_hidden_dim": tuning_hidden,
                        "adv_label_dro_num_layers": tuning_layers,
                        "adv_label_dro_epochs": tuning_epochs,
                    },
