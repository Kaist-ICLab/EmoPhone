"""Group DRO MLP pipeline with Optuna tuning and freeze/unfreeze variants."""
...
class GroupDROBackbone(nn.Module):
...
class GroupDROPipeline:
    def __init__(self, config: GroupDROConfig):
        if optuna is None:
            raise ImportError("optuna is required to run the Group DRO pipeline")
        self.config = config
    ...
    def _evaluate_dataset(self, model: GroupDROBackbone, dataset: Optional[TensorDataset]) -> Dict[str, float]:
        if dataset is None:
    ...
    def _build_model(self, hidden_dim: int, num_layers: int) -> GroupDROBackbone:
        model = GroupDROBackbone(
    ...
    def run(
        ...
        *,
        ...
        results: List[GroupDROSingleRunResult] = []
        ...
        def _run_variant(mode: str, freeze: bool, scratch: bool, seed_offset: int) -> None:
            ...
            results.append(
                GroupDROSingleRunResult(
                    ...
                    scratch=scratch,
                    hyperparams={
                        "group_dro_hidden_dim": tuning_hidden,
                        "group_dro_num_layers": tuning_layers,
                        "group_dro_epochs": tuning_epochs,
                    },
